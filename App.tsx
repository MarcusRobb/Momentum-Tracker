...
import { PlusIcon, TrashIcon, ArrowUpCircleIcon, ArrowDownCircleIcon, DollarSignIcon, CalendarIcon, RestoreIcon, ChevronDownIcon, LightbulbIcon, SparkleIcon, ChevronUpIcon, DownloadIcon, UploadIcon } from './components/Icons';
...
const App: React.FC = () => {
  ...
  const handleToggleSelectMode = () => {
    setIsSelectMode(v => !v);
    setSelectedBrainDumpIds(new Set());
  };
  ...
  const handleSelectAll = () => {
    if (selectedBrainDumpIds.size === brainDump.length) {
      setSelectedBrainDumpIds(new Set());
    } else {
      setSelectedBrainDumpIds(new Set(brainDump.map(t => t.id)));
    }
  };
  ...
  const handleExportCSV = () => {
    ...
    const rows = tasks.map(t => [
      ...
      escapeCsvField(t.scheduledTime || ''),
      String(t.duration || ''),
      String(t.isIncomeGenerating),
    ].join(','));
    triggerDownload(
  ...
  const handleExportTXT = () => {
    ...
    const txt = tasks.map((t, i) => {
      let line = `${i + 1}. ${t.text}`;
      if (t.notes) line += `\n   Notes: ${t.notes}`;
      if (t.scheduledTime) line += `\n   Time: ${t.scheduledTime}`;
      if (t.duration) line += `\n   Duration: ${t.duration} mins`;
      if (t.isIncomeGenerating) line += `\n   [Income Generating]`;
      return line;
    }).join('\n\n');
    triggerDownload(
      txt,
      `brain-dump-${new Date().toISOString().split('T')[0]}.txt`,
      'text/plain'
    );
  };
  ...
  const handleDownloadTemplate = () => {
    const headers = ['Task Name', 'Notes', 'Scheduled Time (HH:MM)', 'Duration (mins)', 'Income Generating (true/false)'];
    const example1 = ['Example Task 1', 'Some action points here', '09:00', '60', 'false'];
    const example2 = ['Example Task 2', 'Follow up with client', '14:00', '30', 'true'];
    triggerDownload(
  ...
  const handleImportCSV = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (ev) => {
      ...
      if (rows.length < 2) {
        alert('CSV appears empty or only has headers.');
        return;
      }
      const header = rows[0].map(h => h.trim().toLowerCase());
      const nameIdx = header.findIndex(h => h.includes('task'));
      ...
      const incomeIdx = header.findIndex(h => h.includes('income'));

      const newTasks: Task[] = rows.slice(1)
        .filter(row => nameIdx >= 0 && row[nameIdx]?.trim())
        .map(row => ({
          id: crypto.randomUUID(),
          text: row[nameIdx].trim(),
          notes: notesIdx >= 0 ? (row[notesIdx]?.trim() || '') : '',
          scheduledTime: timeIdx >= 0 ? (row[timeIdx]?.trim() || '') : '',
          duration: durationIdx >= 0 ? (parseInt(row[durationIdx]) || 60) : 60,
          isIncomeGenerating: incomeIdx >= 0 ? row[incomeIdx]?.trim().toLowerCase() === 'true' : false,
          isCompleted: false,
        }));

      if (newTasks.length === 0) {
        alert('No valid tasks found. Make sure your CSV has a "Task Name" column.');
        return;
      }
      setBrainDump(prev => [...newTasks, ...prev]);
      alert(`Imported ${newTasks.length} task${newTasks.length === 1 ? '' : 's'} into your Brain Dump.`);
    };
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
            ...
            <section aria-labelledby="brain-dump-heading" className="bg-white p-6 rounded-lg shadow-md">
                <div className="flex items-center justify-between mb-4 gap-2 flex-wrap">
                    <h2 id="brain-dump-heading" className="text-2xl font-bold text-slate-800">🧠 Brain Dump</h2>
                        ...
                        {!isSelectMode ? (
                            <>
                                <button
                                    ...
                                    className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium rounded-md bg-slate-100 text-slate-600 hover:bg-slate-200"
                                >
                                ...
                                <button
                                    ...
                                    title="Download CSV template to fill in"
                                    className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium rounded-md bg-slate-100 text-slate-600 hover:bg-slate-200"
                                >
                                ...
                                {brainDump.length > 0 && (
                                    <button
                                        ...
                                        className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium rounded-md bg-blue-50 text-blue-600 hover:bg-blue-100"
                                    >
                                        Export
                                    </button>
                                )}
                            ...
                            <>
                                <label className="flex items-center gap-1.5 text-xs text-slate-600 cursor-pointer select-none">
                                    <input
                                        type="checkbox"
                                        checked={brainDump.length > 0 && selectedBrainDumpIds.size === brainDump.length}
                                        onChange={handleSelectAll}
                                        className="h-4 w-4 rounded border-gray-300 text-blue-600"
                                    />
                                    All
                                </label>
                                <span className="text-xs text-slate-500">{selectedBrainDumpIds.size} selected</span>
                                <button
                                    onClick={handleExportCSV}
                                    disabled={selectedBrainDumpIds.size === 0}
                                    title="Export selected tasks as CSV"
                                    className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium rounded-md bg-green-50 text-green-700 hover:bg-green-100 disabled:opacity-40 disabled:cursor-not-allowed"
                                >
                                    <DownloadIcon className="w-3.5 h-3.5" />
                                    CSV
                                </button>
                                <button
                                    ...
                                    disabled={selectedBrainDumpIds.size === 0}
                                    title="Export selected tasks as plain text"
                                    className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium rounded-md bg-green-50 text-green-700 hover:bg-green-100 disabled:opacity-40 disabled:cursor-not-allowed"
                                >
                                    <DownloadIcon className="w-3.5 h-3.5" />
                                    TXT
                        ...
                        brainDump.map(task => (
                            <div key={task.id} className="flex items-start gap-2">
                                {isSelectMode && (
                                    <input
                                        type="checkbox"
                                        checked={selectedBrainDumpIds.has(task.id)}
                                        onChange={() => handleToggleSelectId(task.id)}
                                        className="mt-4 h-5 w-5 rounded border-gray-300 text-blue-600 flex-shrink-0 cursor-pointer"
